...
async function main() {
  ...
  const { insertedId: adminId } = await db.collection('admins').insertOne({ email: 'admin@kiyorabank.local', passwordHash: await bcrypt.hash('Admin123!', 10), name: 'Kiyora Operations', createdAt: new Date() })
  const customers = [
  ...
  for (const [index, item] of customers.entries()) {
    ...
    for (const [accountIndex, balance] of item.balances.entries()) {
      const { insertedId: accountId } = await db.collection('accounts').insertOne({ userId: String(userId), name: accountIndex ? 'Savings Account' : 'Personal Account', type: accountIndex ? 'SAVINGS' : 'PERSONAL', currency: 'JPY', accountNumber: `KYR${String(4821 + index * 137 + accountIndex * 237).padStart(8, '0')}`, balance, status: 'ACTIVE', createdAt: new Date() })
      await db.collection('transactions').insertOne({ userId: String(userId), accountId: String(accountId), type: 'CREDIT', description: 'Opening balance', senderName: 'Kiyora Bank', amount: balance, currency: 'JPY', status: 'COMPLETED', createdAt: new Date() })
    }
    await db.collection('notifications').insertOne({ userId: String(userId), title: 'Welcome to Kiyora Bank', body: 'Your personal banking profile is ready.', read: false, createdAt: new Date() })
  }
  ...
  await db.collection('transactions').insertOne({ userId: String(user._id), accountId: String(account._id), type: 'CREDIT', description: 'Incoming Transfer', senderName: 'Yamato Holdings', amount: 500000, currency: 'JPY', status: 'COMPLETED', createdAt: new Date() })
